...
/** 全車両の記録（給油・整備・費用）を時系列で一覧表示する履歴画面。ヘッダーはスクロールに追従する。 */
@Composable
    ...
    LaunchedEffect(viewModel) {
        viewModel.effects.collect { effect ->
            when (effect) {
                ...
                is RecordsListEffect.OpenTrip -> onNavigateToTrip(effect.vehicleId)
            }
        }
    }
...
private fun RecordsListScreenPreview() {
    val section = RecordSection(
        ...
        entries = listOf(
            ...
            CostEntry(
                CostRecord(
                    id = 3, vehicleId = 2, date = LocalDate.of(2026, 7, 1),
